...
from zunis.models.layers.trainable import ArbitraryShapeRectangularDNN, FullyConnectedUNetDNN, ArbitraryShapeRectangularTinyCudaNN, ArbitraryShapeRectangularOneBlob
...
def piecewise_quadratic_transform(x, wv_tilde, compute_jacobian=True):
    ...
    assert(check_tensor(w_tilde, "w_tilde", False))
    assert(check_tensor(v_tilde, "v_tilde", False))
    if False:
        w=torch.exp(w_tilde)
        ...
        max_w_tilde = torch.max(w_tilde, dim=-1, keepdim=True)[0]

        # Subtract the max to prevent overflow and exponentiate
        w = torch.exp(w_tilde - max_w_tilde)
        assert(check_tensor(w, "w", False))

        # Sum and normalize
        wsum = torch.cumsum(w, axis=-1)
        assert(check_tensor(wsum, "wsum", False))
        wnorms = torch.unsqueeze(wsum[:, :, -1], axis=-1)
        assert(check_tensor(wnorms, "wnorms", False))
        w = w / wnorms
        assert(check_tensor(w, "w", False))

        # Recompute wsum using normalized w for consistent subsequent operations
        wsum = torch.cumsum(w, axis=-1)
    ...
    assert(check_tensor(wsum_shift, "wsum_shift", False))
    v=modified_softmax(v_tilde, w)
    assert(check_tensor(v, "v", False))
    ...
    assert(check_tensor(mx, "mx", False))
    ...
    assert(check_tensor(alphas, "alphas", False))
    ...
    assert(check_tensor(vw, "vw", False))
    #quadratic term
    ...
    assert(check_tensor(out_1, "out3", False))
    assert(check_tensor(out_2, "out1", False))
    assert(check_tensor(out_3, "out2", False))

    out=out_1+out_2+out_3
    assert(check_tensor(out, "out", False))
    ...
    if compute_jacobian:
        if False:
            ...
            # Ensure all tensors are of the same type, assuming half precision is required
            v_half = v.float()  # Convert v to half precision
            alphas_half = alphas.float()  # Ensure alphas are also in half precision
            assert(check_tensor(v_half, "v_half", False))
            assert(check_tensor(alphas_half, "alphas_half", False))

            # Gather and squeeze operations, ensuring all are in half precision
            gathered_v0 = torch.gather(v_half, -1, mx).squeeze(axis=-1)
            gathered_v1 = torch.gather(v_half, -1, mx + 1).squeeze(axis=-1)
            assert(check_tensor(gathered_v0, "gathered_v0", False))
            assert(check_tensor(gathered_v1, "gathered_v1", False))

            # Perform linear interpolation
            interpolated = torch.lerp(gathered_v0, gathered_v1, alphas_half)
            assert(check_tensor(interpolated, "interpolated", False))


            # Compute the product over all transformed dimensions and log
            logj = torch.log(torch.prod(interpolated, axis=-1)).squeeze(axis=-1)
    ...
    assert(check_tensor(out, "out", False))
    assert(check_tensor(logj, "logj", False))
    return out, logj
...
def piecewise_quadratic_inverse_transform(y, wv_tilde, compute_jacobian=True):
    ...
    print(100)
    d = (b**2) - (2*a*c)
    assert(check_tensor(a, "a", False))
    assert(check_tensor(c, "c", False))
    assert(check_tensor(d, "d", False))
    ...
    assert(check_tensor(sol1, "sol1", False))
    assert(check_tensor(sol2, "sol2", False))

    # choose solution which is in the allowed range

    sol=torch.where((sol1>=0)&(sol1<1), sol1, sol2)
    assert(check_tensor(sol, "sol", False))
    ...
    assert(check_tensor(edges, "edges", False))
...
class PWQuadraticCoupling(GeneralPWQuadraticCoupling):
    ...
    def __init__(self, *, d, mask,
        ...
        if model == "unet":
            self.T = FullyConnectedUNetDNN(d_in=d_in,
                                        out_shape=(d_out, 2*n_bins+1),
                                        hidden_dims=[256, 128, 64, 32])
        if model == "oneblob":
            self.T = ArbitraryShapeRectangularOneBlob(
                                              d_in_y_n=d_in,
                                              d_in_opt_feats=d_opt_feats,
                                              out_shape=(d_out, 2*n_bins+1),
                                              d_hidden=d_hidden,
                                              n_hidden=n_hidden,
        ...
        elif model == "tiny_cuda_nn":
            self.T = ArbitraryShapeRectangularTinyCudaNN(d_in=d_in+d_opt_feats,
                                              out_shape=(d_out, 2*n_bins+1),
                                              d_hidden=d_hidden,
                                              n_hidden=n_hidden,
                                              input_activation=input_activation,
            ...
            self.T = ArbitraryShapeRectangularDNN(d_in=d_in+d_opt_feats,
                                              out_shape=(d_out, 2*n_bins+1),
                                              d_hidden=d_hidden,
                                              n_hidden=n_hidden,
                                              input_activation=input_activation,

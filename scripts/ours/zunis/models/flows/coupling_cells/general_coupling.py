...
class GeneralCouplingCell(GeneralFlow):
    ...
    def transform_and_compute_jacobian(self, yj, opt_feats):
        ...
        if opt_feats is None:
            ...
            T_input = torch.cat((y_n, opt_feats), dim=1)
            my_n = self.T(T_input)

        x[..., self.mask_complement], log_jy = self.transform(y_m, my_n, compute_jacobian=True)
        x[..., -1] = log_j + log_jy
        return x

    def flow(self, y, opt_feats):
        ...
        x[..., self.mask_complement], _ = self.transform(y_m, my_n, compute_jacobian=False)

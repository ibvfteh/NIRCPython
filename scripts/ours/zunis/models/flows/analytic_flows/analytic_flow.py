...
class AnalyticFlow(GeneralFlow):
    ...
    def flow(self, x, opt_feats=None):
        return self.flow_(x, opt_feats)
...
class InvertibleAnalyticFlow(GeneralFlow):
    ...
    def flow(self, x, opt_feats=None):
        if self.inverse:
            return self.inverse_flow_.flow(x, opt_feats)

        return self.flow_.flow(x, opt_feats)

    def transform_and_compute_jacobian(self, xj, opt_feats=None):
        if self.inverse:
            return self.backward_flow.transform_and_compute_jacobian(xj, opt_feats)

        return self.forward_flow.transform_and_compute_jacobian(xj, opt_feats)

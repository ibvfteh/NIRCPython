...
class BasicTrainer(ABC):
    ...
    def train_step_on_target_minibatch(self, x, px, fx, optim, opt_feats=None, add_pdf=None, zero_grad=True, div=None, est=None):
        ...
        if add_pdf is not None:
            k = torch.log(add_pdf.unsqueeze(-1))
        else:
            k = torch.zeros(x.shape[0], 1).to(x.device)



        xj = torch.cat([x, k], dim=1)

        zj = self.flow(xj, opt_feats=opt_feats)


        z = zj[:, :-1] # log prob of the flow, jacobian

        #assert(check_tensor(z, "prior", False, 0.0, 1.0))
        #assert(check_tensor(zj[:, -1], "jacobian", False))
        logqx = zj[:, -1] + self.latent_prior.log_prob(z) # log prob of the prior

        check_tensor(logqx, "logqx", force_analyze=False)
        if zero_grad:
            optim.zero_grad()
        loss = self.loss(fx, px, logqx, div, est)
        return loss

    def train_step_on_target_batch(self, x, px, fx, optim, minibatch_size=None, opt_feats=None):
    ...
    def train_on_target_batch(self, x, px, fx, optim, n_epochs, minibatch_size=None, opt_feats=None):
        ...
        for epoch in range(n_epochs):
            ...
            try:
                self.train_step_on_target_batch(x, px, fx, optim, minibatch_size, opt_feats=opt_feats)
    ...
    def train_on_target_batches_from_posterior(self,
        ...
        for batch in range(n_batches):
            ...
            self.train_on_target_batch(x, px, fx, optim, n_epochs_per_batch, minibatch_size, opt_feats=opt_feats)
...
class BasicStatefulTrainer(BasicTrainer, GenericTrainerAPI):
    ...
    def train_step_on_target_minibatch(self, x, px, fx, optim, opt_feats=None, add_pdf=None, zero_grad=True, div=None, est=None):
        loss = super(BasicStatefulTrainer, self).train_step_on_target_minibatch(x, px, fx, optim, opt_feats=opt_feats, add_pdf=add_pdf, zero_grad=zero_grad, div=div, est=est)
        #self.record.next_step()
        return loss

    def train_step_on_target_batch(self, x, px, fx, optim, minibatch_size=None, opt_feats=None):
        self.record.new_epoch()
        super(BasicStatefulTrainer, self).train_step_on_target_batch(x, px, fx, optim, minibatch_size, opt_feats=opt_feats)
    ...
    def train_on_batch(self, x, px, fx, opt_feats=None, **kwargs):
        ...
        self.train_on_target_batch(x, px, fx, optim=optim, n_epochs=n_epochs, minibatch_size=minibatch_size, opt_feats=opt_feats)
        return self.record
